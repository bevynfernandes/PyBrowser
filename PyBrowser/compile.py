...
def main():
	...
	config.smain("PyBrowser/config.json")
	print(f"{vars=}")
	command = f'pyinstaller --noconfirm --onefile --{vars["app_type"]} --icon "{cwd}/images/icon.ico" --name "{vars["name"]}" --clean --key "{secrets.token_urlsafe()}" --add-data "{cwd}/images;images/" --add-data "{cwd}/config.json;." "{cwd}/main.py"'
